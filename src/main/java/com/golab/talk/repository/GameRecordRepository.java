...
public interface GameRecordRepository extends JpaRepository<GameRecord, String> {
	...
	//나와 상대의 전적 조회
	@Query(value = "SELECT A.user_id AS userId, A.games, A.wins, A.mmr, RANK() OVER(ORDER BY MMR DESC, WINS DESC) ranking FROM game_record A WHERE user_id = ?1 OR user_id = ?2", nativeQuery = true)
	List<Map<String, Object>> findMMRWithRanking(String userId1, String userId2);
