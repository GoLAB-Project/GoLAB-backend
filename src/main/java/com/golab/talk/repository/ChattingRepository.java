...
public interface ChattingRepository extends JpaRepository<Chatting, Integer> {
	...
	int countByRoomIdAndIdGreaterThan(int roomId, int lastReadChatId);

	// notRead 관련 쿼리 제거 - Participant.lastReadChatId 로 읽음 상태 관리
	@Modifying
	@Transactional
	@Query(value = "insert into chatting(room_id, send_user_id, message, created_at, updated_at) VALUES(?1, ?2, ?3, NOW(), NOW())", nativeQuery = true)
